...
class Lite(GDRN_Lite):
    ...
    def run(self, args, cfg_teacher,cfg_student):
        
        
        cfg_student.OUTPUT_DIR=cfg_teacher.OUTPUT_DIR
        cfg_student.MODEL.WEIGHTS=""
        cfg_student.DISTILLATION.FLAG=True
        cfg_student.DISTILLATION.SEGMENTATION=cfg_teacher.DISTILLATION.SEGMENTATION
        
        
        self.set_my_env(args, cfg_student)
        
        

        # get renderer ----------------------
        if args.eval_only or cfg_student.TEST.SAVE_RESULTS_ONLY or (not cfg_student.MODEL.POSE_NET.XYZ_ONLINE):
            renderer = None
        else:
            train_dset_meta = MetadataCatalog.get(cfg_student.DATASETS.TRAIN[0])
            data_ref = ref.__dict__[train_dset_meta.ref_key]
            train_obj_names = train_dset_meta.objs
            render_gpu_id = self.local_rank
            renderer = get_renderer(cfg_student, data_ref, obj_names=train_obj_names, gpu_id=render_gpu_id)
        
        logger.info(f"Used GDRN module teacher name: {cfg_teacher.MODEL.POSE_NET.NAME}")
        model, optimizer = eval(cfg_teacher.MODEL.POSE_NET.NAME).build_model_optimizer(cfg_teacher, is_test=True)
        logger.info("Model:\n{}".format(model))


        logger.info(f"Used GDRN module student name: {cfg_student.MODEL.POSE_NET.NAME}")
        model2, optimizer2 = eval(cfg_student.MODEL.POSE_NET.NAME).build_model_optimizer(cfg_student, is_test=False)
        logger.info("Model:\n{}".format(model))
        ...
        model_student, optimizer_student = self.setup(model2, optimizer2)
        ...
        if True:
            ...
            params_student = sum(p.numel() for p in model_student.parameters()) / 1e6
            logger.info("{}M params".format(params_student))
            

        print("Loading weights for the teacher model")  # save results only ------------------------------
        MyCheckpointer(model, save_dir=cfg_teacher.OUTPUT_DIR, prefix_to_remove="_module.").resume_or_load(
                cfg_teacher.MODEL.WEIGHTS, resume=args.resume)

        print("Starting_distillation")
        self.do_distill(cfg_student,cfg_teacher,args, model,model_student, optimizer_student, renderer=renderer, resume=args.resume)
        if hard_limit < FILE_LIMIT:
            logger.warning("set sharing strategy for multiprocessing to file_system")
            torch.multiprocessing.set_sharing_strategy("file_system")
        #return self.do_test(cfg, model2)


@loguru_logger.catch
def main(args):
    cfg= setup(args)
    args.student_config=cfg.DISTILLATION.STUDENT
    
    cfg_student=setup(args,student=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--student_config",default=None,type=str)
