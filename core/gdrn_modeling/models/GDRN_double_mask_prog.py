...
import os
import csv
class GDRN_DoubleMask_prog(nn.Module):
    def __init__(self, cfg, backbone, geo_head_net, neck=None, pnp_net=None):
        super().__init__()
        assert cfg.MODEL.POSE_NET.NAME == "GDRN_double_mask_prog", cfg.MODEL.POSE_NET.NAME
...
def build_model_optimizer(cfg,is_test=False):
    ...
    # build model
    model = GDRN_DoubleMask_prog(cfg, backbone, neck=neck, geo_head_net=geo_head, pnp_net=pnp_net)
    if net_cfg.USE_MTL:
        params_lr_list.append(
            {
                "params": filter(
