...
class TopDownDoubleMaskXyzRegionHeadProg(nn.Module):
    def __init__(
        self,
        in_dim,
        ...
        for i, up_type in enumerate(up_types):
            ...
            num_conv_per_block=get_conv_per_block(i,layer)
        ...
        self.conv_layer_index= get_index_conv(len(up_types),num_conv_per_block)
    ...
    def forward(self, x):
        ...
        for i, l in enumerate(self.features): 
            if i==(self.layer-1) and self.layer!=3:
                x = (1-self.alpha)*x+self.alpha*l(x)
                if self.alpha!=1:
                    self.alpha+=0.001
...
def get_index_conv(upsample,conv_per_block):
    layers=upsample+upsample*conv_per_block+2
    list=[x for x in range(0,layers)]
    return list

def get_conv_per_block(upsample,layer):
    if upsample==0:
        if layer ==3:
            conv_per_block=0
        elif layer ==4:
